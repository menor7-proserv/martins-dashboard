...
async function calcDre(mes: number, ano: number) {
  ...
  for (const d of despesasPorCat) {
    catMap[d.categoria] = d._sum.valor ?? 0
  }
  ...
  const [atual, historico] = await Promise.all([
    ...
    Promise.all(
      Array.from({ length: 6 }, (_, i) => {
        ...
        return calcDre(d.getMonth() + 1, d.getFullYear())
      })
    ),
  ])
